...
export default function Page() {
  ...
  const copyEmail = async () => {
    try {
      ...
      window.location.href = 'mailto:hello.onmark.support@gmail.com'
    }
  }
  useEffect(() => {
  ...
  return <main>
    ...
    <section className="enquiry shell section-grid reveal-on-scroll" id="enquiry"><div className="section-label"><span>08</span><span>YOUR BRAND</span></div><div className="enquiry-content"><h2>Start with <em>your brand.</em></h2>{submitted ? <div className="form-success"><span>THANK YOU</span><h3>Your enquiry is ready to review.</h3><p>We&apos;ll be in touch about your custom bottle direction.</p><button className="text-action" type="button" onClick={() => setSubmitted(false)}>Send another enquiry <span>＋</span></button></div> : <form className="enquiry-form" onSubmit={handleSubmit}><div className="form-grid"><label>Business / brand name<input required name="business" maxLength={120} placeholder="Your business name" /></label><label>Your name<input required name="name" maxLength={120} placeholder="Contact person" /></label><label>Email<input required type="email" name="email" maxLength={254} placeholder="you@business.com" /></label><label>Phone / WhatsApp<input required type="tel" name="phone" maxLength={30} pattern="[+0-9 ()-]{7,30}" placeholder="+91 ..." /></label></div><label>Business type<select name="type" defaultValue="Restaurant"><option>Restaurant</option><option>Café</option><option>Hotel / Resort</option><option>Wedding / Event</option><option>Corporate event</option><option>Other</option></select></label><label>Additional requirements<textarea name="message" rows={4} maxLength={1200} placeholder="Tell us a little about your bottle project" /></label><label className="upload-label">Logo upload<input type="file" name="logo" accept="image/png,image/jpeg,image/webp,application/pdf" /></label><button className="button" type="submit">Get your bottle designed <span>↗</span></button></form>}</div></section>
